...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _caricaStatoTimbratura() async {
    final oggi = DateTime.now();
    final ultima = await DatabaseService.getUltimaTimbraturaPerData(
              ...
              IconButton(
                ...
                onPressed: () async {
                  await Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => NotificheScreen(dipendente: widget.dipendente),
                    ),
                  );
                  _loadNotificheCount();
                },
              ),
              if (_notificheNonLette > 0)
                Positioned(
                  right: 8,
                  top: 8,
                  child: Container(
...
// Mappa
              ...
              Row(
                ...
                children: [
                  ...
                  _buildTimbraButton(
                    ...
                    Icons.logout,
                    () => _timbra('uscita'),
                    enabled: _ultimaTimbraturaTipo == 'entrata' ||
                        _ultimaTimbraturaTipo == 'pausa_fine',
                  ),
                ],
              ),
              const SizedBox(height: 24),
              Row(
                ...
                children: [
                  ...
                  _buildTimbraButton(
                    ...
                    Icons.coffee_maker,
                    () => _timbra('pausa_fine'),
                    enabled: _ultimaTimbraturaTipo == 'pausa_inizio',
                  ),
                ],
              ),
  ...
  Widget _buildStatoCard() {
    String stato;
    IconData icon;
    Color color;
    String descrizione;

    if (_ultimaTimbraturaTipo == null) {
      stato = 'Giornata non iniziata';
      icon = Icons.play_circle_outline;
      color = Colors.grey;
      descrizione = 'Premi ENTRATA per iniziare';
    } else if (_ultimaTimbraturaTipo == 'entrata') {
      stato = 'Al lavoro';
      icon = Icons.work;
      color = Colors.green;
      descrizione = 'Entrata alle ${DateFormat('HH:mm').format(_ultimaTimbraturaData!)}';
    } else if (_ultimaTimbraturaTipo == 'pausa_inizio') {
      stato = 'In pausa';
      icon = Icons.coffee;
      color = Colors.orange;
      descrizione = 'Pausa iniziata alle ${DateFormat('HH:mm').format(_ultimaTimbraturaData!)}';
    } else if (_ultimaTimbraturaTipo == 'pausa_fine') {
      stato = 'Ripreso lavoro';
      icon = Icons.work;
      color = Colors.blue;
      ...
      child: Row(
        children: [
          ...
          IconButton(
            ...
            onPressed: _caricaStatoTimbratura,
          ),
        ],
      ),
    );
  ...
  Widget _buildTimbraButton(
    ...
    return ElevatedButton(
      ...
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(icon, size: 24, color: enabled ? Colors.white : Colors.grey.shade500),
